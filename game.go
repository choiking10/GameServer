	...
	for {
		...
		case s := <-h.Unregister:
			if _, ok := h.Sessions[s]; ok {
				...
				//if holdPoint {
					//i := len(tmpPlayer)
					tmpPlayer =append(tmpPlayer, *s.Player)
					//playerchecker =true
					//tmpPlayer[i]=*s.Player
					delete(h.Sessions, s)
				//}
				s.c.Close()
			}
...
func PositionFromInt(x, y int) Position {
	//	fmt.Println("In PositionFromInt")
	return Position{float64(x), float64(y)}
}

func (p Position) RoundX() int {
	//fmt.Println("In Position.RoundX")
	return int(p.X + 0.5)
}

func (p Position) RoundY() int {
	//fmt.Println("In Position.RoundY")
	return int(p.Y + 0.5)
}
...
func (p *Player) addTrailSegment(pos Position, marker rune) {
	//	fmt.Println("In addTrailSegment ")
...
func (gm *GameManager) getGameWithAvailability() *Game {
	//		fmt.Println("In getGameWithAvailability")
	var g *Game
...
func (gm *GameManager) SessionCount() int {
	//		fmt.Println("In GameManager.SessionCount")
	sum := 0
	for _, game := range gm.Games {
...
func (gm *GameManager) GameCount() int {
	//		fmt.Println("In GameManager.GameCount")
	return len(gm.Games)
}

/*func (gm *GameManager) HandleExistChannel(c ssh.Channel, color string){
	session := Session{c: c, LastAction: time.Now()}
	for s := range gm.Games["a"].hub.Sessions{
		player := s.Player
...
func (gm *GameManager) HandleNewChannel(c ssh.Channel, color string) {
	//		fmt.Println("In GameManager.HandleNewChannel")
	g := gm.getGameWithAvailability()

	if g == nil {
		//fmt.Println("game availability nil")
	...
	/*if !holdPoint{
		...
		session.Player.Name = color
		fmt.Printf("color :%s, Player name : %s\n", color,session.Player.Name)
	//}
	go func() {
		...
		//	if(totalSessionCount>=6){
			//	inputchecker++
	//		}
		//	if(totalSessionCount>=6 && inputchecker<11){
			//	fmt.Printf("input delay time : %s \n",elapsedTime)
		//	}
		}
	}()
}
...
func (g *Game) setTileType(pos Position, tileType TileType) error {
	//fmt.Println("In game.setTileType")
...
func (g *Game) worldString(s *Session) string {
	//		fmt.Println("In game.worldString")
...
func (g *Game) WorldWidth() int {
	//		fmt.Println("In game.worldWidth")
	return len(g.level)
}

func (g *Game) WorldHeight() int {
	//		fmt.Println("In game.worldHeight")
	return len(g.level[0])
}

func (g *Game) AvailableColors() []color.Attribute {
	//		fmt.Println("In game.availableColors")
...
func (g *Game) SessionCount() int {
	//		fmt.Println("In game.SessionCount")
...
func (g *Game) Update(delta float64) {
	...
	for player, session := range g.players() {
		...
		//	fmt.Printf("player : %s ,coordStr : %s \n",player.Color,coordStr)
		}
		/*for key,val := range trailCoordMap{
			fmt.Println(key,val)
		}
...
var(
	outputmoduleChecker = 0
)

func (g *Game) Render(s *Session) {
	//		fmt.Println("In game.render")
	...
	elapsedTime := time.Since(startTime)
	if(totalSessionCount >=6){
		if(outputmoduleChecker < 11){
			fmt.Printf("output module time : %s\n",elapsedTime)
		}
		outputmoduleChecker++
	}
}
...
func (g *Game) AddSession(s *Session) {
	//		fmt.Println("In game.AddSession")d
	g.hub.Register <- s
}

func (g *Game) RemoveSession(s *Session) {
	//fmt.Println("In game.RemoveSession")
	g.hub.Unregister <- s
}
